  ...
  function GridItem({ app }: { app: any }) {
    return (
      <div className="text-muted-foreground hover:bg-muted flex w-full cursor-pointer items-center gap-4 rounded-md p-2 transition-all">
        ...
        <div className="text-left">{app.appName}</div>
      </div>
    );
  }
  ...
  return (
    <>
      <p className="mb-4 text-sm">
        {apps.length} {apps.length === 1 ? "app/service" : "apps/services"}:
      </p>
      <div className="mb-8 grid grid-cols-2 gap-4 text-sm sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4">
        {apps.map((app) => (
          <div key={app.appName}>
            {isDesktop ? (
              <Dialog>
